...
import { InvoiceClient, ChantierInfo } from "@/lib/invoices/types";
...
interface StepClientProps {
  ...
  onClientChange: (client: InvoiceClient | null) => void;
...
export function StepClient({ client, chantier, onClientChange, onChantierChange }: StepClientProps) {
  ...
  const handleClientChange = (field: keyof InvoiceClient, value: any) => {
    if (!client) {
      onClientChange({
        type: "particulier",
        name: "",
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <Label htmlFor="clientType" className="text-white">Type</Label>
            <Select
              value={client?.type || "particulier"}
              onValueChange={(value) => handleClientChange("type", value)}
            >
              <SelectTrigger className="bg-black/20 border-white/10 text-white">
                <SelectValue />
              </SelectTrigger>
          ...
          <div>
            <Label htmlFor="clientVatNumber" className="text-white">N° TVA intracommunautaire</Label>
            <Input
              id="clientVatNumber"
              value={client?.vatNumber || ""}
              onChange={(e) => handleClientChange("vatNumber", e.target.value)}
              className="bg-black/20 border-white/10 text-white"
              placeholder="FR XX XXX XXX XXX (B2B)"
            />
          </div>
          ...
          <div className="md:col-span-2">
            <div className="flex items-center space-x-2 mb-2">
              <input
                type="checkbox"
                id="differentDelivery"
                checked={hasDifferentDeliveryAddress}
                onChange={(e) => setHasDifferentDeliveryAddress(e.target.checked)}
                className="rounded"
              />
              <Label htmlFor="differentDelivery" className="text-white cursor-pointer">
          ...
          {hasDifferentDeliveryAddress && (
            <>
              <div className="md:col-span-2">
                <Label htmlFor="deliveryAddress" className="text-white">Adresse de livraison</Label>
                <Input
                  id="deliveryAddress"
              ...
              <div>
                <Label htmlFor="deliveryPostalCode" className="text-white">Code postal</Label>
                <Input
                  id="deliveryPostalCode"
                  value={client?.deliveryPostalCode || ""}
                  onChange={(e) => handleClientChange("deliveryPostalCode", e.target.value)}
                  className="bg-black/20 border-white/10 text-white"
                  placeholder="75001"
                  maxLength={5}
                />
              ...
              <div>
                <Label htmlFor="deliveryCity" className="text-white">Ville</Label>
                <Input
                  id="deliveryCity"
                  value={client?.deliveryCity || ""}
                  onChange={(e) => handleClientChange("deliveryCity", e.target.value)}
                  className="bg-black/20 border-white/10 text-white"
                  placeholder="Paris"
                />
              </div>
