...
import { QuoteLine, QuoteLot, generateId, UnitEnum, VatRateEnum } from "@/lib/quotes/types";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Button } from "@/components/ui/button";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Plus, Trash2, GripVertical } from "lucide-react";
import { calculateLineTotal } from "@/lib/quotes/calculations";
...
interface StepLinesProps {
  lots: QuoteLot[];
  lines: QuoteLine[];
  onLotsChange: (lots: QuoteLot[]) => void;
...
export function StepLines({ lots, lines, onLotsChange, onLinesChange }: StepLinesProps) {
  ...
  const removeLot = (lotId: string) => {
    ...
    // Retirer les lignes du lot supprimé
  ...
  const updateLine = (lineId: string, field: keyof QuoteLine, value: any) => {
    const updatedLines = lines.map(line => {
      if (line.id === lineId) {
        ...
        // Recalculer si quantité ou prix change
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/70">Ajoutez les prestations de votre devis</p>
        </div>
              ...
              <div className="flex items-center justify-between mb-4">
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    // Mettre à jour les noms de lots dans les lignes
                    onLinesChange(lines.map(line =>
      ...
      {lines.length === 0 && (
        <div className="text-center py-12 text-white/50">
          <p>Aucune ligne de devis. Cliquez sur "Ajouter ligne" pour commencer.</p>
        </div>
      )}
    </div>
  );
...
function LineRow({
  ...
  line: QuoteLine;
  lots?: QuoteLot[];
  onUpdate: (field: keyof QuoteLine, value: any) => void;
  onRemove: () => void;
