...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  apiRouter.post('/devis/:id/envoyer-signature', async (req, res) => {
    ...
    try {
      ...
      const signwellPayload = {
        ...
        // Pour descendre, augmenter y (y=842 est le bas de la page)
        fields: [
          [
            {
              type: 'signature',
              ...
              page: 1,
              x: 420,
              y: 800,  // Proche du bas (842 - 60 hauteur - marge) - zone "Lu et approuvé"
              width: 200,
              height: 60
            },
            {
              ...
              page: 1,
              x: 420,
              y: 750,  // Juste au-dessus de la signature
              width: 150,
              height: 30
            }
          ]
  ...
  apiRouter.post('/webhooks/signwell', async (req, res) => {
    try {
      ...
      if (!supabaseServiceKey) {
        ...
        return res.status(500).json({ error: 'Configuration Supabase manquante. Veuillez ajouter SUPABASE_SERVICE_ROLE_KEY dans les variables d\'environnement.' });
      }
  ...
  // Route POST /devis/:id/update-signature-status pour mettre à jour le statut de signature
  apiRouter.post('/devis/:id/update-signature-status', async (req, res) => {
    try {
      const { id } = req.params;
      const { status, signwellDocumentId, signwellSignedPdfUrl, dateSignature, dateRefus } = req.body;

      // Cette route est appelée par le frontend après avoir reçu le webhook
      // Le frontend met à jour directement Supabase, cette route est juste pour confirmer
      res.json({
        success: true,
